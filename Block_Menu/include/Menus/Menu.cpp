...
void Menu::Update(glm::vec2 mousePos)
{
	for (uint i = 0; i < m_Windows.size(); i++)
	{
		MenuWindow* tempWindow = m_Windows.at(i); // position is screen space coords
		glm::vec2 windowPos = tempWindow->GetPosition();
		glm::vec2 windowSize = tempWindow->GetSize();
...
void Menu::LeftClick(glm::vec2 mousePos)
{
	for (uint i = 0; i < m_Windows.size(); i++)
	{
