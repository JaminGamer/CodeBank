#ifndef __MENUWINDOW_H_
#define __MENUWINDOW_H_
...
public:
	...
	void SetSpacing(glm::vec2 spacing) { m_Spacing = spacing; };
...
protected:
	...
	glm::vec2 m_Spacing = glm::vec2(10.0f, 10.0f); // non-Uniform spacing?
	...
	glm::vec2 m_Position = glm::vec2(1, 1);
	glm::vec2 m_Size = glm::vec2(1, 1);
	glm::vec2 m_Resolution = glm::vec2(g_WindowWidth, g_WindowHeight);
	float m_Angle = 0.0f;
	...
	void CenterItems();
	// Gizmos
	void MovementGizmo(glm::vec2 mousePos);
	void ScalingGizmo(glm::vec2 mousePos);
	bool m_IsInteracting = false; // TODO: Determine when mouse is interacting with Gizmos
};

#endif // __MENUWINDOW_H_