...
MenuWindow::MenuWindow(ShaderProgram* shader, Mesh* mesh, int items)
{
	...
	for (int i = 0; i < items; i++) // Create and add items
	{
		...
		tempItem->SetPosition(glm::vec2(0.5, 0.5));
		tempItem->SetSize(glm::vec2(0.25, 0.25));
		tempItem->SetRotation(0.0f);
		tempItem->SetColor(glm::vec4(RandFloatInRange(0.0f, 1.0f), RandFloatInRange(0.0f, 1.0f), RandFloatInRange(0.0f, 1.0f), 1.0f));
		tempItem->SetParentWindow(this);

		m_Items.PushBack(tempItem);
	}
	m_Color = glm::vec4(0.5, 0.5, 0.5, 1); // Background color
}
...
void MenuWindow::Update(glm::vec2 mousePos)
{
	// Update gizmos
	...
	for (int i = 0; i < m_Items.Size(); i++)
	{
		...
		glm::vec2 itemPosition = tempItem->GetPosition();
		glm::vec2 itemSize = tempItem->GetSize();

		glm::vec2 finalSize = itemSize * m_Size;
...
void MenuWindow::UpdateTransform()
{
	...
	temp = glm::rotate(temp, glm::radians(m_Angle), glm::vec3(0, 0, 1));

	temp = glm::translate(temp, glm::vec3(m_Position.x / m_Resolution.x * 2 - 1, m_Position.y / m_Resolution.y * 2 - 1, 0));

	glm::vec3 t = glm::vec3(m_Size.x * 2.0f / m_Resolution.x, m_Size.y * 2.0f / m_Resolution.y, 1);

	temp = glm::scale(temp, t);
...
void MenuWindow::ResizeItems()
{
	...
	for (int i = 0; i < m_Items.Size(); i++)
	{
		m_Items.At(i)->SetSize(glm::vec2(horizontalSize / m_Size.x, verticalSize / m_Size.y)); // percent
		m_Items.At(i)->UpdateTransform();
	}
}
...
{
	glm::vec2 t_ItemSize = m_Items.At(0)->GetSize(); // assume consistent size for all items
	glm::vec2 t_Position = glm::vec2(0, 0);
	...
	for (int i = 0; i < m_Rows; i++)
	{
		...
		for (int j = 0; j < m_Columns; j++)
		{
			...
			// position as percent%
			m_Items.At(t_ItemIndex)->SetPosition(glm::vec2((t_Position.x / m_Size.x) + shiftValueX, (1 - t_Position.y / m_Size.y) - shiftValueY));
...
void MenuWindow::LeftClick(glm::vec2 mousePos)
{
	for (int i = 0; i < m_Items.Size(); i++)
	{
		MenuItem* tempItem = m_Items.At(i); // item's position is in window position % of dimensions(center +/- % of width/height)
...
void MenuWindow::SetResolution(glm::vec2 resolution)
{
	for (int i = 0; i < m_Items.Size(); i++)
	{
...
/* Private Functions */
void MenuWindow::MovementGizmo(glm::vec2 mousePos)
{
	...
	{
		if (debug_GetPlayerController()->IsButtonPressed(eGameButtons::Button_MouseRight))
		{
			vec2 t_ = debug_GetPlayerController()->GetMousePosDelta();
			m_Position += glm::vec2(t_.x, t_.y);
			UpdateTransform();
		}
	}
}

void MenuWindow::ScalingGizmo(glm::vec2 mousePos)
{
	// Update scale gizmo
	vec2 t_Position = vec2(m_Position.x, m_Position.y);
	vec2 t_Size = vec2(m_Size.x, m_Size.y);
	...
	if (PointInBox(vec2(mousePos.x, mousePos.y), t_GizmoPos, m_ScaleGizmo.size.x, m_ScaleGizmo.size.y))
	{
		...
		{
			vec2 t_ = debug_GetPlayerController()->GetMousePosDelta();
			m_Size += glm::vec2(t_.x * 2.0f, t_.y * -2.0f);
			UpdateTransform();
		}
	}
}